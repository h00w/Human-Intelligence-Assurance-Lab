...
st.caption("Frozen release evidence and current operational qualification are intentionally shown separately.")

operational_adaptive = load_json("runs/adaptive_hedging_latest.json")
...
c1.metric("Frozen Phase 1.8", PHASE_1_8_AUTHORITATIVE["decision"])
...
st.caption(
    f"Phase 1.8 authoritative source: GitHub Actions run {PHASE_1_8_AUTHORITATIVE['workflow_run_id']} / "
    f"artifact {PHASE_1_8_AUTHORITATIVE['artifact_id']}. Later operational runs do not rewrite this historical verdict."
)

latest_decision = (
    operational_adaptive.get("executive_decision", {}).get("decision") if operational_adaptive else None
)
if latest_decision:
    if latest_decision == "SHIP":
        st.info("Latest operational adaptive qualification: SHIP")
    else:
        st.error(
            f"Latest operational adaptive qualification: {latest_decision}. "
            "This is current longitudinal evidence and must remain visible even though the frozen Phase 1.8 release was SHIP."
        )
...
if readiness.ready:
    ...
    st.warning("The historical Phase 1.8 release remains valid, but Phase 1 v1.0 maturity is not yet satisfied.")
    for blocker in readiness.blockers:
        st.write(f"- {blocker}")

st.subheader("Human semantic calibration")
st.write(
    "Release threshold: ≥20 resolved samples, ≥2 independent reviewers/sample, inter-reviewer κ ≥0.70, "
    "judge-vs-human κ ≥0.70, critical-failure recall ≥0.95."
)
if calibration:
    st.json(
        {
...
if long_window:
    ...
    if long_window.get("status") == "HOLD":
        st.error("At least one longitudinal snapshot violated the full production/resilience contract.")
else:
    st.info(
        "No mature time-separated series exists yet. Weekly qualification archives dated snapshots; insufficient history is WAITING, while a production-contract regression is HOLD."
    )
...
st.divider()
st.caption(
    "Phase 1 v1.0 cannot be made green by lowering safety/SLO thresholds, fabricating human labels, "
    "discarding unfavorable operational evidence, or silently provisioning paid infrastructure."
)
