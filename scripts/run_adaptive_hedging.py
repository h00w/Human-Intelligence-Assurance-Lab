...
def balanced_risk_sample(scenarios, per_risk: int = 6):
    ...
    for risk in ("critical", "high", "medium", "low"):
        out.extend([s for s in scenarios if s.risk_level == risk][:per_risk])
    return out
...
def main() -> None:
    ...
    def adaptive_adapter(primary_adapter=None):
        return AdaptiveHedgedAdapter(
            primary_adapter or adapter(model, primary, token),
            adapter(model, fallback, token, timeout_s=fallback_timeout_s),
            policy=policy,
            risk_by_prompt=risk_by_prompt,
        )

    adaptive_report = run_model_evaluation(
        adaptive_adapter(), eval_scenarios, system_prompt_builder=risk_aware_system_prompt
    )
    ...
    fault_reports = []
    for trial in range(1, fault_trials + 1):
        ...
        report = run_model_evaluation(
            adaptive_adapter(timeout_primary),
            critical_cases,
            system_prompt_builder=risk_aware_system_prompt,
        )
        report["trial"] = trial
    ...
    fault_pass = all(eligible(report) for report in fault_reports)
    ...
    payload = {
        ...
        "comparison": {
            ...
            "fixed_1500ms": {"report": fixed_report, "economics": fixed_econ},
        ...
        "executive_decision": {
            ...
            "reasons": [
                ...
                "all repeated critical timeout recoveries passed"
                if fault_pass
                else "one or more critical timeout recoveries failed",
                "repeated qualification passed" if stability.stable else "repeated qualification failed",
                (
                    "adaptive redundant cost <= fixed 1.5 s"
                    if cost_improved
                    else "adaptive redundant cost exceeded fixed 1.5 s; efficiency evidence only"
                ),
            ],
        },
    }
        ...
        json.dumps(
            {
                ...
                "critical_fault_all_passed": fault_pass,
