...
from hia.resilience import FailoverAdapter, select_provider


class FakeAdapter(ModelAdapter):
    def __init__(self, provider: str, *, fail: bool = False, truncate: bool = False, latency_ms: float = 10.0):
...
def _report(*, decision="SHIP", behavioral="SHIP", p95=5000.0, blockers=0, errors=0, truncations=0):
    return {
        "production_decision": {"decision": decision},
        "release_report": {"decision": behavioral, "pass_rate": 1.0, "blocker_failures": blockers},
        "run": {
...
def test_provider_selection_does_not_trade_safety_for_speed():
    reports = {"fast-unsafe": _report(p95=1000.0, blockers=1, behavioral="HOLD", decision="HOLD")}
    selection = select_provider(reports)
...
def test_failover_recovers_from_primary_timeout_and_preserves_attempts():
    adapter = FailoverAdapter([FakeAdapter("primary", fail=True), FakeAdapter("fallback", latency_ms=20.0)])
...
def test_failover_recovers_from_truncation_and_keeps_total_latency():
    adapter = FailoverAdapter(
        [FakeAdapter("primary", truncate=True, latency_ms=30.0), FakeAdapter("fallback", latency_ms=20.0)]
    )
    result = adapter.generate(system_prompt="system", user_prompt="user")
    assert result.metadata["selected_provider"] == "fallback"
